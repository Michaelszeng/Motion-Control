 ...
 *
 * UNDERGOING TESTING WITH PATH SPLICING (USING STABLE TIME-BASED MOTION PROFILING AND PID)
 */
@TeleOp(name = "PurePursuitFollowerV5")
public class PurePursuitFollowerV5 extends LinearOpMode {
    String TAG = "PurePursuitFollowerV5";
    ...
    ArrayList<PurePursuitPath> allPaths = new ArrayList<>();
    ArrayList<PurePursuitMotionProfileGenerator> allMPGs = new ArrayList<>();
    ...
    int pathIndex = -1;     //This is incremented at the beginning of the program to 0
...
//        allPaths.add(new PurePursuitPath(path, "pp_path_2.xml"));
        allPaths.add(new PurePursuitPath(new ArrayList<>(), "path_splice_test2_path_1.xml"));
        allPaths.add(new PurePursuitPath(new ArrayList<>(), "path_splice_test2_path_2.xml"));

        RobotLogger.dd(TAG, "allPaths:");
        for (PurePursuitPath path : allPaths) {
            RobotLogger.dd(TAG, path.toString1());
        }
...
//        backLeft = hwMap.backLeft;
        ...
        while (opModeIsActive()) {   //put teleop code in here
            //EXECUTED ONCE PER PATH
            if (!pathInitialized) {
                pathDuration = 0.0;
                prevTargetIndex = 0;
                pathIndex += 1;
                PPPath = allPaths.get(pathIndex);
                RobotLogger.dd(TAG, "PPPath: " + PPPath.toString1());

                pathEnd = PPPath.path1.get(PPPath.path1.size()-1);

                allMPGs.add(new PurePursuitMotionProfileGenerator(PPPath));
//            PurePursuitMotionProfileGenerator mpg = new PurePursuitMotionProfileGenerator(PPPath);
                allMPGs.get(pathIndex).generateProfiles();
            ...
            //EXECUTED SEVERAL TIMES PER PATH
            ...
            pathDuration = new Date().getTime() - pathStartDate.getTime();
            RobotLogger.dd(TAG, "Current Duration: " + duration);
            RobotLogger.dd(TAG, "Path Duration: " + pathDuration);
            ...
            RobotLogger.dd(TAG, "PPPath right before Pure Pursuit Algorithm: " + PPPath.toString1());
            ...
            motorPowers = robot.update(backRight.getCurrentPosition(), backLeft.getCurrentPosition(), frontRight.getCurrentPosition(), imuReading, currentTarget, targetIndex, PPPath, (int) dateDiff, pathDuration/1000);
...
//        right_back.setDirection(DcMotorSimple.Direction.REVERSE);
    ...
    private void detectEnd(Pose2d currentPose) {
        reachedDestination = PurePursuitMathFunctions.reachedDestination(currentPose, pathEnd, reachedDestination, pathDuration, PPPath.path1.get(PPPath.path1.size()-1).t);
        if (reachedDestination == PurePursuitMathFunctions.ReachedDestination.TRUE) {
            RobotLogger.dd(TAG, "========================================================REACHED DESTINATION========================================================");
            if (pathIndex == allPaths.size()-1) {   //If the last path in the program has finished:
                exportTelemetryData();
                this.stop();
            }
            pathInitialized = false;    //This prompts the path index to increment, moving onto the next path
        }
    }
