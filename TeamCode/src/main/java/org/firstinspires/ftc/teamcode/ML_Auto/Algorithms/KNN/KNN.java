...
public class KNN implements Serializable {
    ...
    private int k;
    private List<KNNData> data;

    public KNN(int k){
        this.k = k;
    }

    public void train(List<KNNData> data){
        this.data = data;
    }

    public double classify(double[] x){
        List<Result> resultList = new ArrayList<Result>();
        for(KNNData knn : data){
            double dist = 0.0;
            for(int j = 0; j < knn.x.length; j++){
                dist += Math.pow(knn.x[j] - x[j], 2) ;
                //System.out.print(city.cityAttributes[j]+" ");
            }
            double distance = Math.sqrt( dist );
            resultList.add(new Result(distance,knn.label));
        ...
        for(int i = 0; i < k; i++){
            System.out.println(resultList.get(i).label+ " .... " + resultList.get(i).distance);
            //get classes of k nearest instances (city names) from the list into an array
            ss[i] = resultList.get(i).label;
        }

        return meanOfArray(ss);
    }
   ...
   /* public static void main(String args[]){
    ...
    static class Result {
        double distance;
        double label;
        public Result(double distance, double label){
            this.label = label;
