...
import static org.firstinspires.ftc.teamcode.util.PurePursuitMathFunctions.getError;
...
public class PurePursuitFollowerV4_7_Traditional_PID_Testing extends LinearOpMode {
    ...
    //Currently this data doesn't do anything
    ...
    public static double prevY = 0.0;
    public static double prevH = 0.0;
    public static double prevV = 0.0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        PPPath = new PurePursuitPath(path, "scenario3.xml");
...
//        backLeft = hwMap.backLeft;
        ...
        while (opModeIsActive()) {   //put teleop code in here
            ...
            duration = (double) (dateNew.getTime() - startDate.getTime())/1000;
...
//            motorPowers = robot.update(backRight.getCurrentPosition(), backLeft.getCurrentPosition(), frontRight.getCurrentPosition(), imuReading, currentTarget, targetIndex, PPPath, (int) dateDiff, duration/1000);
            ...
            errorHist.add(getError(targetIndex, PPPath, currentPose));

            detectEnd(currentPose);

            packet.put("x", currentPose.getX());
            packet.put("y", currentPose.getY());
            packet.put("h", currentPose.getHeading());
...
//        right_back.setDirection(DcMotorSimple.Direction.REVERSE);
    ...
    private void detectEnd(Pose2d currentPose) {
        reachedDestination = PurePursuitMathFunctions.reachedDestination(currentPose, pathEnd, reachedDestination, duration*1000, PPPath.path1.get(PPPath.path1.size()-1).t);
        if (reachedDestination == PurePursuitMathFunctions.ReachedDestination.TRUE) {
            exportTelemetryData();
            exportTelemetryDataErrors();
            this.stop();
        }
    }
...
//            ReadWriteFile.writeFile(logFile, MPString);
    ...
    private void exportTelemetryDataErrors() {
        if (errorHist.size() > 0) {
            String tString = "";
            String errorsString = "";
            for (int i=0; i<errorHist.size(); i++) {
                tString += tHist.get(i) + ",";
                errorsString += errorHist.get(i) + ",";
            }
            String allTelemetry = "t:" + tString + "\n" + "e:" + errorsString;
            ReadWriteFile.writeFile(logFileErrors, allTelemetry);
            RobotLogger.dd(TAG, "Successfully wrote to errors telemetry file.");
        }
    }
}
